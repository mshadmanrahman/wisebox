...
function renderBlock(block: ContentBlock, index: number) {
  switch (block.type) {
    ...
    case 'heading':
      if (block.level === 2) {
        return (
          <h2 key={index} className="text-xl font-semibold text-white mt-8 mb-3">
            {block.text}
          </h2>
        );
      }
      return (
        <h3 key={index} className="text-lg font-medium text-white mt-6 mb-2">
          {block.text}
        </h3>
      );
      ...
      return (
        <div key={index} className="space-y-1">
          {block.terms.map((term, i) => (
            ...
            >
              <span className="text-white font-medium">{term.bn}</span>
